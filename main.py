...
def Sampling_RL(actor, critic, inputs, vector, length, epsilon, Random = True):
    ...
    for pos in range(length):
        ...
        if Random:
            ...
            # action = np.argmax(predicted).cpu().item() #这里加了.numpy() ,因为报错
            action = np.argmax(predicted.cpu().detach().numpy()).item() #如果这里不这么写，False走进来会报错
            ...
            if RL_train:
                ...
                for i in range(samplecnt):
                    ...
                    if (steps) % 50 == 0:
                        criticModel.eval()
                        actorModel.eval()
                        act, _, _, _ = Sampling_RL(actorModel, criticModel, x, criticModel.wordvector_find(x), length, epsilon, Random=False) #False这里出的错
                        # print(act, "\n\n") #
                        criticModel.train()
                        actorModel.train()
